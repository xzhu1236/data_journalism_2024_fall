...
```{r}
md_top_return <- md_absentee |>
  mutate(
    returned_per = (total_received/total_sent)*100
  ) |>
  arrange(desc(returned_per)) |>
  select(county_name, returned_per) |>
  head(10)

md_top_return |>
  ggplot() +
  geom_bar(aes(x = reorder(county_name, returned_per), weight = returned_per)) +
  coord_flip() +
  labs(
    title = "Counties ordered by percentage of total ballots returned",
    x = "County",
...
```{r}
md_low_return <- md_absentee |>
  mutate(
    returned_dem = (dem_received/dem_sent)*100,
    returned_rep = (rep_received/rep_sent)*100,
    difference_ret = (returned_dem - returned_rep)
  ) |>
  arrange(returned_dem) |>
  select(-geoid, -oth_sent, -total_sent, -oth_received, -total_received) |>
  head(10)
  
md_low_return |>
  ggplot() +
  geom_bar(aes(x = reorder(county_name, returned_dem), weight = returned_dem)) +
  coord_flip() +
  labs(
    title = "Counties ordered by percentage of Democrat ballots returned",
    x = "County",
    y = "Percent of ballots returned",
    caption = "Source: Maryland State Board of Elections"
  )
...
**A3** People send in more absentee balot votes the closer it is to election day. Democrats are the most by far.

```{r}
frederick <- md_absentee_date |>
  filter(county_name == "FREDERICK")

frederick |>
  ggplot(aes(x=date)) + 
  geom_line(aes(y = dem_received, color = "Democrat")) +
  geom_line(aes(y = rep_received, color = "Republican")) +
  geom_line(aes(y = oth_received, color = "Other")) +
  labs(
    title = "Number of ballots returned by date",
    x = "Date",
...
**A4.** Honestly, it looks like most counties fall on the line of best fit well

```{r}
md_absentee |>
  ggplot() +
  geom_point(aes(x=total_sent, y=total_received)) +
  geom_text(aes(x = total_sent, y = total_received, label = county_name), angle = -45, size = 2, vjust = -2) +
  geom_smooth(aes(x = total_sent, y = total_received), method = 'lm') +
  labs(
    title = "Number of ballots sent in comparison to number returned",
    x = "Sent",
    y = "Returned",
    caption = "Source: Maryland State Board of Elections"
  )
...
Chatpgpt conversation: https://chatgpt.com/share/671ff9ee-2f0c-800f-860a-9bd9fe8bd421
