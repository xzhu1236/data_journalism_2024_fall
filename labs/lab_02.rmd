...
```{r}
view_md_voter_county_24 <- (md_voters_2024 |>
  group_by(COUNTY) |>
  select(COUNTY, FIRSTNAME, LASTNAME, GENDER, PARTY, STATE_REGISTRATION_DATE, COUNTY_REGISTRATION_DATE) |>
...
```{r}
region_md_voters_24 <- (md_voters_2024 |>
  mutate(
    REGION = case_when(
      ...
      COUNTY == "Worchester" ~ "Eastern Shore Maryland",
      COUNTY == "Cecil" ~ "Eastern Shore Maryland",
    )
  ) |>
  group_by(COUNTY) |>
  select(COUNTY, REGION, FIRSTNAME, LASTNAME, GENDER, PARTY, STATE_REGISTRATION_DATE, COUNTY_REGISTRATION_DATE) |>
  filter(COUNTY_REGISTRATION_DATE == STATE_REGISTRATION_DATE) |>
  filter(COUNTY_REGISTRATION_DATE >= "2024-7-21") |>
  arrange(COUNTY))
```
