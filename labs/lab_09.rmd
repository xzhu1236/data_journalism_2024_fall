...
**A1.** Mail-in ballots spoked a lot during the 2020 elections. 
https://datawrapper.dwcdn.net/M7RJI/1/

```{r}
md_vote_type <- read_csv("data/md_votes_by_type.csv") |>
  mutate(
    Early_per = (Early/Total)*100,
    Election_per = (`Election Day`/Total)*100,
    Mail_per = (Mail/Total)*100
  )

write_csv(md_vote_type, "data/md_votes_by_type.csv")
```
...
**A2.** What day had the most amount of young people turnout?
https://datawrapper.dwcdn.net/YcR3r/1/
...
**A3.** I would want to know more about how the data was gathered because there was a huge difference in the Montgomery county numbers between WinRed and actblue. The most interesteting story is probably looking at the difference in numbers in Dorchester county because actblue has 1 donation in that area.
https://datawrapper.dwcdn.net/YHKFG/1/
https://datawrapper.dwcdn.net/7b1LE/1/
...
```
```{r}
md_winred_pop <- md_winred |>
  left_join(md_pop, join_by(zip == GEOID)) |>
  mutate(per_capita = (total/estimate)*1000)

md_actblue_pop <- md_actblue |>
  left_join(md_pop, join_by(zip == GEOID)) |>
  mutate(per_capita = (total/estimate)*1000)

write_csv(md_winred_pop, "data/md_winred_zip_totals.csv")
write_csv(md_actblue_pop, "data/md_actblue_zip_totals.csv")
```
