...
```{r}
voters_by_county <- read_csv("data/Tabula_data_eligible_voters.csv")
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer** Some of the rows don't have labels and we have no idea what the columns mean
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer** The headers are correct now but there are still blank rows
